...
router.post('/llm/test', async (req, res) => {
    ...
    try {
        const { provider, apiKey, model, temperature = 0.7, maxTokens = 100, timeout = 30000 } = req.body;
...
async function testOpenAIProvider(apiKey, model = 'gpt-3.5-turbo', temperature = 0.7, maxTokens = 100, timeout = 30000) {
    if (!apiKey) {
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [{
            role: 'user',
            content: 'Test: Are you working? Respond with "OpenAI connection successful for music recommendations!"'
        }],
        temperature,
...
async function testGeminiProvider(apiKey, model = 'gemini-1.5-flash', temperature = 0.7, maxTokens = 100, timeout = 30000) {
    if (!apiKey) {
...
async function testAnthropicProvider(apiKey, model = 'claude-3-haiku-20240307', temperature = 0.7, maxTokens = 100, timeout = 30000) {
    if (!apiKey) {
    ...
    const response = await axios.post('https://api.anthropic.com/v1/messages', {
        ...
        messages: [{
            role: 'user',
            content: 'Test: Are you working? Respond with "Anthropic Claude connection successful for music recommendations!"'
        }]
    }, {
        headers: {
            'x-api-key': apiKey,
...
async function testOpenRouterProvider(apiKey, model = 'anthropic/claude-3-haiku', temperature = 0.7, maxTokens = 100, timeout = 30000) {
    if (!apiKey) {
    ...
    const response = await axios.post('https://openrouter.ai/api/v1/chat/completions', {
        ...
        messages: [{
            role: 'user',
            content: 'Test: Are you working? Respond with "OpenRouter connection successful for music recommendations!"'
        }],
        temperature,
        max_tokens: maxTokens
    }, {
