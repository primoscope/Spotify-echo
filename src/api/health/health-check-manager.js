...
class HealthCheckManager {
    ...
    try {
      ...
      try {
        const databaseManager =
          global.databaseManager || require('../../database/database-manager');
  ...
  async checkAIProviders() {
    ...
    const overallStatus =
      healthyCount > 0
        ? 'healthy'
        : Object.values(results).some((r) => r.status === 'degraded')
          ? 'degraded'
          : 'unhealthy';
    ...
    return {
      ...
      last_full_check:
        this.lastResults.size > 0
          ? Math.max(
              ...Array.from(this.lastResults.values()).map((r) => new Date(r.last_check).getTime())
            )
          : null,
    };
  }
