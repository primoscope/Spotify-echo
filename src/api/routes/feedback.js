...
router.get('/analytics', requireAuth, async (req, res) => {
  try {
    ...
    const analytics = await feedbackCollection
      .aggregate([
        {
          $match: {
            userId: req.userId,
    ...
    const feedbackData = analytics[0] || {
      totalFeedback: 0,
      averageRating: 0,
      feedbackBreakdown: [],
    };
...
async function processFeedbackForLearning(feedbackDoc) {
  try {
    ...
    console.log(
      `Processing feedback for learning: ${feedbackDoc.type} - ${feedbackDoc.feedback || feedbackDoc.rating}`
    );
