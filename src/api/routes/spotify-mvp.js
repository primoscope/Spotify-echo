...
router.get('/playlists', requireSpotifyAuth, async (req, res) => {
  try {
    ...
    const playlists = await makeSpotifyRequest(
      '/me/playlists',
      req.auth.spotifyTokens.access_token,
      {
        params: {
...
router.get('/recently-played', requireSpotifyAuth, async (req, res) => {
  try {
    ...
    const recentTracks = await makeSpotifyRequest(
...
router.get('/audio-features/:trackIds', requireSpotifyAuth, async (req, res) => {
  try {
    ...
    const audioFeatures = await makeSpotifyRequest(
      '/audio-features',
      req.auth.spotifyTokens.access_token,
      {
        params: {
