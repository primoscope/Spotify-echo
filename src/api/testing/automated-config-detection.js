...
class AutomatedConfigDetector extends events_1.EventEmitter {
    ...
    async analyzeRepository() {
        ...
        try {
            ...
            if (await this.fileExists(cursorRulesPath)) {
                structure.cursorRules = await fs_1.promises.readFile(cursorRulesPath, 'utf-8');
            }
            ...
            if (await this.directoryExists(cursorRulesDir)) {
                const ruleFiles = await fs_1.promises.readdir(cursorRulesDir);
            ...
            if (await this.fileExists(mcpConfigPath)) {
                structure.mcpConfig = JSON.parse(await fs_1.promises.readFile(mcpConfigPath, 'utf-8'));
            }
    ...
    async applyConfiguration(config, options = {
        createBackups: true,
    ...
    async detectProjectType(structure) {
        ...
        try {
            const files = await fs_1.promises.readdir(this.rootPath);
    ...
    async detectTestingFrameworks(structure) {
        const frameworks = [];
        const deps = { ...structure.dependencies.production, ...structure.dependencies.development };
        const testingMap = {
            'jest': 'Jest',
    ...
    getLanguageSpecificRules(languages) {
        const rules = {};
    ...
    getFrameworkSpecificRules(frameworks) {
        const rules = {};
    ...
    getTestingRules(testingFrameworks) {
        const rules = [
    ...
    getPerformanceRules(projectType) {
        const rules = [
    ...
    generateContextManagement(structure) {
        const config = {
            includePatterns: [
                '**/*.{js,ts,jsx,tsx}',
                '**/*.{md,mdx}',
    ...
    generateEnvironmentVariables(structure) {
        const variables = [];
        // Common variables
        variables.push({
            name: 'NODE_ENV',
    ...
    async applyCursorRules(config, options) {
        ...
        await fs_1.promises.writeFile(rulesPath, rulesContent);
        this.emit('file_created', { path: rulesPath });
    }
    // Apply MCP configuration
    async applyMCPConfiguration(config, options) {
        const mcpConfig = {
            mcpServers: {}
        };
    ...
    // Apply context management configuration
    async applyContextConfiguration(config, options) {
        const contextConfig = {
        ...
        await fs_1.promises.writeFile(contextPath, JSON.stringify(contextConfig, null, 2));
        this.emit('file_created', { path: contextPath });
    }
    // Generate environment template
    async generateEnvironmentTemplate(config, options) {
    ...
    // Apply recommended scripts
    async applyRecommendedScripts(config, options) {
        if (!this.repositoryStructure?.packageJson)
            return;
    ...
    generateCursorRulesContent(config) {
...
exports.AutomatedConfigDetector = AutomatedConfigDetector;
