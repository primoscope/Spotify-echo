                ...
                {Object.keys(testResults).length > 0 && (
                    ...
                    >
                        <Typography variant="body2">
                            <strong>Test Results for {testResults.provider}:</strong><br />
                            Status: {testResults.status}<br />
                            {testResults.latency && `Latency: ${testResults.latency}ms`}<br />
                            Response: {testResults.response}<br />
        ...
        <Card>
            <CardContent>
                ...
                <Box sx={{ mb: 3 }}>
                    ...
                    <Chip 
                        icon={settings.database.connectionStatus === 'connected' ? <CheckIcon /> : <ErrorIcon />}
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} md={4}>
                            <Paper sx={{ p: 2 }}>
                                ...
                                <Typography variant="h6">
                                    {settings.database.avgQueryTime || 'N/A'}
                                </Typography>
                            </Paper>
                        </Grid>
                ...
                <DialogContent>
                    {loading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', p: 3 }}>
                            <CircularProgress />
                            <Typography sx={{ ml: 2 }}>
                                Testing {settings.llm.provider} connection...
                            </Typography>
                        </Box>
                    ) : (
                        Object.keys(testResults).length > 0 && (
                            <Box>
                                <Alert severity={testResults.status === 'success' ? 'success' : 'error'}>
                                    <Typography variant="body1">
                                        ...
                                        <strong>Status:</strong> {testResults.status}<br />
                                        {testResults.latency && (
                                            <>
                                                <strong>Response Time:</strong> {testResults.latency}ms<br />
                                            </>
                                        )}
                                        <strong>Response:</strong> {testResults.response}<br />
