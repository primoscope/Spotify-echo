...
class SlowRequestLogger {
  ...
  getPerformanceStatus(summary) {
    if (summary.criticalRequests > 0) {
      return 'critical';
    } else if (summary.verySlowRequests > summary.totalRequests * 0.05) {
      // More than 5% very slow
      return 'warning';
    } else if (summary.slowRequestPercentage > 20) {
      // More than 20% slow
      return 'degraded';
...
const slowRequestLogger = new SlowRequestLogger({
  ...
  logFilePath:
    process.env.SLOW_REQUEST_LOG_PATH || path.join(process.cwd(), 'logs', 'slow-requests.log'),
});
