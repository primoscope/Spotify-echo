...
class HealthChecker {
  ...
  async checkDatabase() {
    ...
    const allConnected =
      (!results.mongodb.configured || results.mongodb.connected) &&
      (!results.redis.configured || results.redis.connected);

    return {
      healthy: !hasConfiguredDbs || allConnected,
      message: !hasConfiguredDbs
        ? 'No databases configured'
        : allConnected
          ? 'All configured databases connected'
  ...
  async checkSystem() {
    ...
    return {
      ...
      message:
        warnings.length > 0 ? `System warnings: ${warnings.join(', ')}` : 'System resources normal',
      details: {
        memory: {
  ...
  async checkStorage() {
    const pathsToCheck = [process.cwd(), './logs', './tmp', './public'];
    ...
    return {
      ...
      message: allPathsAccessible
        ? 'All storage paths accessible'
        : 'Some storage paths inaccessible',
      details: results,
    };
  }
